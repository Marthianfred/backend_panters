...
export class PostgresVideoChatRepository implements IVideoChatRepository {
  ...
  async createSession(
    session: Partial<VideoCallSession>,
  ...
  async getUserSessions(userId: string): Promise<VideoCallSession[]> {
    const query = `
      SELECT id, creator_id as "creatorId", user_id as "userId", schedule_time as "scheduleTime",
             duration_minutes as "durationMinutes", price_coins as "priceCoins", status, stream_id as "streamId"
  ...
  async updateSessionStatus(
    ...
    const query =
      'UPDATE video_call_sessions SET status = $1, updated_at = NOW() WHERE id = $2;';
    ...
    try {
      ...
      const creatorWalletQuery = `
        ...
        ON CONFLICT (creator_id) DO UPDATE SET
          ...
          updated_at = CURRENT_TIMESTAMP;
      `;
      await client.query(creatorWalletQuery, [
        creatorId,
        netAmount,
        platformCommission,
      ]);
