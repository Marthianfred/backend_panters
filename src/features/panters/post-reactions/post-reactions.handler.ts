  ...
  async execute(
    ...
    if (!exists) {
      throw new NotFoundException(
        `La publicación con ID ${dto.postId} no existe en el muro.`,
      );
    }
    ...
    if (!creatorId) {
      throw new NotFoundException(
        'No se pudo encontrar a la creadora de esta publicación.',
      );
    }

    const totalPanteras = await this.repository.upsertReaction(
      userId,
      dto.postId,
    );
    ...
    return {
      ...
      message:
        'Pantera acumulada. Esta reacción será procesada por nuestro sistema de rating.',
    };
  }
}
